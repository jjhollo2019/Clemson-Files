...
public class Game extends AppCompatActivity implements DialogFragment1.DialogFragmentListener {
    ...
    public boolean checkWin(){
        ...
        if(PLAYER_BANK >= SCORE_TO_WIN || COMPUTER_BANK >= SCORE_TO_WIN){
            if(PLAYER_BANK >= SCORE_TO_WIN){
                TextView textView = findViewById(R.id.winnerString);
                textView.setText(PLAYER_WON);
            }
            else {
                TextView textView = findViewById(R.id.winnerString);
                textView.setText(COMPUTER_WON);
            }
            Winner winner = new Winner();
            getSupportFragmentManager().beginTransaction().add(R.id.frag_win, winner).commit();
            return true;
        }
        //else no one has one
        else
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView1.setText(String.valueOf(ROUND) + "-" + PTurn);
    }
    ...
    /**
     * This function will end the player turn and allow the computer to tke a turn after
     * @pre onCreateOptionsMenu must be called
     * @post the current score is added to the player bank and the computer has taken a turn
     */
    private void playerTurn(){
        ...
        //else the game continues
            ...
            //add to player bank and reset the current score
            ...
            textView.setText(String.valueOf(ROUND) + "-" + CTurn);
            computerTurn();
            System.out.println("end computer turn again");
            //reset for player turn
            textView.setText(String.valueOf(ROUND) + "-" + PTurn);
            PLAYER_TURN = true;
            CURRENT_SCORE = 0;
            textView1.setText(String.valueOf(CURRENT_SCORE));
            ROUND++;
     ...
     * @pre must be called inside the player turn
     * @post the computer score is updated
     */
    public void computerTurn(){
        //roll once and determine if there should be another roll
        boolean rollAgain = true;
        rollDice();
        Random random = new Random();
        int choice = random.nextInt(15);
        //the computer will roll 2/3 times
        while ((choice % 3 == 0 || choice % 3 == 1) && rollAgain) {
            rollDice();
            if (mDice.getNumber() == 1) {
                rollAgain = false;
            } else {
                choice = random.nextInt(15);
            }
        }
        //add to computer bank and reset current score
