...
public interface IGameBoard
{
    ...
    {
        for(int a = getNumRows() - 1; a >= 0; a--){//loop through the rows of the selected column
            //return true if a free space is found
            if( whatsAtPos(a, c) == ' '){
                return true;
            }
        }
        //default to false if no empty spaces are found
        return false;
    }
    ...
    default boolean checkForWin(int c)
    {
        for(int r = getNumRows()-1; r >= 0; r--)
           if(whatsAtPos(r, c) != ' '){
            if (checkDiagWin(r, c, whatsAtPos(r,c)))
                return true;
            if (checkHorizWin(r, c, whatsAtPos(r, c)))
                return true;
            if (checkVertWin(r, c, whatsAtPos(r, c)))
                return true;
           }
    ...
    default boolean checkTie()
    {
        for(int i = 0; i < getNumRows(); i++){
            for(int j = 0; j < getNumColumns(); j++){
                checkHorizWin(i,j,whatsAtPos(i,j));
                checkDiagWin(i,j,whatsAtPos(i,j));
                checkVertWin(i,j,whatsAtPos(i,j));
            }
        }
